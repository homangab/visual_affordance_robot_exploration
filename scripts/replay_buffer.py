...
class ReplayBuffer:
    """
    The base class for a replay buffer
    """
    def __init__(self,                
        ...
        self.state_space_shape = 4*32*32
        self._actions = np.zeros(
            (buffer_size, max_trajectory_length, self.action_space_shape)
        )
        self._states = np.zeros(
            (buffer_size, max_trajectory_length, self.state_space_shape)
        )
    ...
    def add_trajectory(self, states, actions, desired_state, length_of_traj=None):
        ...
        self._internal_goals[self.pointer] = desired_state
    ...
    def _get_batch(self, traj_idxs, time_state_idxs, time_goal_idxs):
        ...
        goals = self._desired_states[traj_idxs, time_state_idxs]
        print("goal",goals.shape)
